$("#jobno").change(function (){
    ...
    filters = [{"name": "jobno", "op":"eq", "val": jobno}]
    let scheduleUrl = `${baseUrl}/api/schedule`
    $.getJSON(scheduleUrl, JSON.stringify(filters), function(data){
        $.each(data.objects, function(i,obj){
            let assignedUrl = `${baseUrl}/api/assigned/${obj.assignno}`
            $.getJSON(assignedUrl, function(data){
